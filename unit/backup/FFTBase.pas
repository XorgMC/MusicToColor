...
type
  ...
  public
   ...
   procedure FFT( var F        :Pointer;
		  ...
		  const Window  :Integer ); //N -ðàçìåð F, M - N =2^M
...
var U, Uo, W, T :TComplex;
    ...
    var i: Integer;
    begin
     if Not Inverse then //Åñëè íå îáðàòíûé
      case Window of  //0-ïðÿìîóãîëüíîå//1-òðåóãîëüíîå//2-Õýìèíãà
       0: begin  //0-ïðÿìîóãîëüíîå
           {for i :=0 to N-1 do
             F[i].Re :=F[i].Re*1.2}
          end;
       1: begin //1-òðåóãîëüíîå
...
begin
 ...
 if not Inverse then //Âû÷èñëåíèå Ìîùíîñòè Ñ=SQRT(À^2+B^2) ïðè -ïðÿìîì- ÁÏÔ
  for i :=0 to N-1 do
   TComplexArray(F^)[i].Re :=sqrt(sqr(TComplexArray(F^)[i].Re)+sqr(TComplexArray(F^)[i].Im));
 DelMem; //Îñâîáîæäåíèå ðåñóðñîâ
end;
