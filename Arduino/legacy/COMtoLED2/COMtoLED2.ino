...
#define ledPin LED_BUILTIN       // светодиод на плате arduino
...
//  TWord akk;
  ...
  if (magn[n]<gain) { magn[n]=gain; bonkFallspeed += 1; } //Gain larger than fading value -> store gain as fading value
               else gain=magn[n]; //Gain smaller than fading value -> let led fade
  cl.dw = pgm_read_dword(&colorTab[96*n/bandPass]);
  cl = mulsGain(cl,gain);
  return strip.Color(cl.r, cl.g, cl.b);
...
/*void zFeeryFrom() {
  ...
  for(i=0; i<bandPass; i++) {
    if (magn[i]<fallspeed) magn[i] = 0;
                        else magn[i] -= fallspeed/2; 
    if (magn[i]<readData[i]) magn[i]=readData[i];
    if (m<magn[i] && magn[i]>2*prvPk && abs(i-prvPkF)<2 ) { m=magn[i]; mb=i; };
  }

  /*for(i=ctrPt; i<bandPass; i++) {
    if (magn[i]<fallspeed) magn[i] = 0;
                        else magn[i] -= fallspeed/2; 
    if (magn[i]*15<readData[i]) magn[i]=min(readData[i]*(1+(i/10)), 255);
    if (m<magn[i]) { m=magn[i]; mb=i; };
  }//
...
void zFeeryFrom() {
  ...
  if(readData[0] > 254) {
    for(k=0; k<bandPass; k++) strip.setPixelColor(k, 16777215);
  }
  strip.show();
}
/*void zFeeryFrom() {
...
void zMidland2() {
  ...
  cl = bandMagn(0);
  for(k=0; k<bandPass; k++) strip.setPixelColor(k, cl);
  strip.show();
}
