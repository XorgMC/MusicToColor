...
#define ledPin 6       // светодиод на плате arduino
#if defined(__AVR_ATmega328P__)  // Arduino UNO, NANO
  #define stripPin 5      // выход управления светодиодной лентой
  #define stripLed 400    // количество светодиодов в ленте
#else
  #define stripPin PIN_NEOPIXEL      // выход управления светодиодной лентой
  #define stripLed 700    // количество светодиодов в ленте
#endif
...
const uint32_t PROGMEM
  colorTab[]={
      0xFF0000,0xFF1100,0xFF2200,0xFF3300,0xFF4400,0xFF5500,0xFF6600,0xFF7700,0xFF8800,0xFF9900,0xFFAA00,0xFFBB00,0xFFCC00,0xFFDD00,0xFFEE00,0xFFFF00,  //red - yellow
      0xFFFF00,0xEEFF00,0xDDFF00,0xCCFF00,0xBBFF00,0xAAFF00,0x99FF00,0x88FF00,0x77FF00,0x66FF00,0x55FF00,0x44FF00,0x33FF00,0x22FF00,0x11FF00,0x00FF00,  //yellow - green
      0x00FF00,0x00FF11,0x00FF22,0x00FF33,0x00FF44,0x00FF55,0x00FF66,0x00FF77,0x00FF88,0x00FF99,0x00FFAA,0x00FFBB,0x00FFCC,0x00FFDD,0x00FFEE,0x00FFFF,  //green - cyan (blue)
      0x00FFFF,0x00EEFF,0x00DDFF,0x00CCFF,0x00BBFF,0x00AAFF,0x0099FF,0x0088FF,0x0077FF,0x0066FF,0x0055FF,0x0044FF,0x0033FF,0x0022FF,0x0011FF,0x0000FF,  //cyan - blue
      0x0000FF,0x1100FF,0x2200FF,0x3300FF,0x4400FF,0x5500FF,0x6600FF,0x7700FF,0x8800FF,0x9900FF,0xAA00FF,0xBB00FF,0xCC00FF,0xDD00FF,0xEE00FF,0xFF00FF,  //blue - purple(magenta)
      0xFF00FF,0xFF00EE,0xFF00DD,0xFF00CC,0xFF00BB,0xFF00AA,0xFF0099,0xFF0088,0xFF0077,0xFF0066,0xFF0055,0xFF0044,0xFF0033,0xFF0022,0xFF0011,0xFF0000,  //magenta - red
...
const TDWord PROGMEM progColor[] = {
  { 0,16,64,80},   // Colors of the first subprogram; 1,2,3,4 colors respectively
  {16,32,64,48},   // Colors of the second subprogram
  {32,64,16,48},
  {48, 0,80,16},
  {64,16,32,80},
  {80,16, 0,48},
  {96,64,48,16},
  {96, 0,32,64}   // Colors of eighth subprogram
  };
/*
0  - red
16 - yellow
...
uint16_t stepN[4]  = {0,0,0,0}; //Max value = stripLed-1
uint16_t waitN[4]  = {3,3,4,4};
uint16_t waitNc[4] = {3,3,4,4};
...
uint8_t subprog = 7;
uint8_t subcolor = 0;
uint16_t progStep = 0; //Max value=StripLed
uint16_t progSubStep = 0; //Max value=StripLed-1
uint8_t progDir = 0;
...
uint8_t nPhase = 0;
uint8_t bonkFallspeed = fallspeed;
int avg = 0;
uint8_t inStr[32];            
uint8_t readData[32];
...
void sub12_1(TColor color) {
  ...
  //Max Value=StripLED-1
  for(uint16_t i=stripLed-1; i>0; i--) strip.setPixelColor(i,strip.getPixelColor(i-1));
...
void sub10() {
  TColor clF,clD;
  uint16_t i; //Max Value=stripLed-1
...
uint32_t bandMagnDbg(uint8_t n) {
//  TWord akk;
  TColor cl;
  uint8_t gain;
...
/*
 * Returns the nth band color from the color table
 * Index in color table = (96 Colors : number of bands) * nth band
 */
...
//  akk.w=readData[n]*brightness;
//  gain=akk.b1;
  gain=(readData[n]*brightness)>>8; //Get brightness-influenced gain
  if (magn[n]<fallspeed) magn[n] = 0;
                    else magn[n] -= fallspeed; //Let brightness fall
  if (magn[n]<gain) magn[n]=gain; //Gain larger than fading value -> store gain as fading value
               else gain=magn[n]; //Gain smaller than fading value -> let led fade
  cl.dw = pgm_read_dword(&colorTab[96*n/bandPass]);
  cl = mulsGain(cl,gain);
  return strip.Color(cl.r, cl.g, cl.b);
}

uint32_t bandMagnBonk(uint8_t n) {
//  TWord akk;
  TColor cl;
  uint8_t gain;
  ...
  if (magn[n]<gain) magn[n]=gain; //Gain larger than fading value -> store gain as fading value
               else { gain=magn[n];  bonkFallspeed += 1; } //Gain smaller than fading value -> let led fade
  cl.dw = pgm_read_dword(&colorTab[96*n/bandPass]);
  cl = mulsGain(cl,gain);
  return strip.Color(cl.r, cl.g, cl.b);
}
...
/*
 * Music mode 1: Rainbow
 * Divides the strip into $bandPass sections colored like the rainbow and
 * modulates brightness according to music
 */
void zRainbow() {
  uint32_t cl;
  uint16_t i,k,n;
...
void zRainbow10() {
  ...
  for(uint8_t i=0; i<bandPass/2; i++) {
      ...
      for(uint16_t k=0; k<stripLed/6; k++) strip.setPixelColor(n+k, cl.dw);
  }
  strip.show();
}
...
void toCenterShift() {
  uint16_t i; //Max value=stripLed/2
  uint32_t c; //Color value
...
void zMagicTo() {
  ...
  j=0;
  for(i=0; i<bandPass; i++) {
    if (j<readData[i]) { j=readData[i]; k=i; }
    sig += readData[i];
  }

  stepN[0] = (((brightness/2)-1)*stepN[0])+min((192*k/bandPass), 96);
  stepN[0] = stepN[0] / (brightness/2);
  ...
  //sig = ((readData[0] + readData[1])/2);

  avg += ( sig - avg) * 0.1;

  avg = min(avg, 255);

  //if(peak < 5) rndNumber1 = random(96);

  //cl.dw = pgm_read_dword(&colorTab[rndNumber1]);
  cl.dw = pgm_read_dword(&colorTab[stepN[0]]);
  cl=mulsGain(cl, min(avg*3, 255));
  cl.dw = strip.Color(cl.r, cl.g, cl.b);
 
  for(i=0; i<stripLed; i++) {
...
//Full bar bass peak detection, 
//color change near "zero crossing"
void zFeeryFrom() {
  TColor cl;
  uint16_t i,k,j;
  uint8_t sig;
  ...
  peak = min(max(0, sig - avg) * (((brightness%126)+20)/20), 255);

  if (magn[0]<fallspeed*1.4) magn[0] = 0;
                    else magn[0] -= fallspeed*1.4; //Let brightness fall
  if (magn[0]<peak) magn[0]=peak; //Gain larger than fading value -> store gain as fading value
               else peak=magn[0];

  if(peak < 5) { if(brightness/2 == 0) { rndNumber1 = random(96); } else { if(rndNumber1>=95) rndNumber1=0; else rndNumber1+=1; } }
...
/*void zFeeryFrom() {
  ...
  if(readData[0] > 254) {
    for(k=0; k<stripLed; k++) strip.setPixelColor(k, 16777215);
  }
  strip.show();
}*/
/*void zFeeryFrom() {
...
void zFeeryTo() {
  ...
  stepN[0] = (24*stepN[0])+min((192*k/bandPass), 96);
  stepN[0] = stepN[0] / 25;
  ...
  peak = min(max(0, sig - avg)* ((brightness+20)/20), 255);
...
/*void zFeeryTo() {
  ...
  peak = min(max(0, sig - avg), 255);

  if(peak > 3*avg) rndNumber1 = random(96);

  cl.dw = pgm_read_dword(&colorTab[rndNumber1]);
  cl=mulsGain(cl, min(peak*3, 255));
  cl.dw = strip.Color(cl.r, cl.g, cl.b);
...
void zAllureFrom() {
  ...
  for(i=0; i<bandPass; i++) {
    sum += readData[i];
      ...
      n=i*LedtoColor;
      for(k=0; k<LedtoColor; k++) strip.setPixelColor(n+k, cl);
      if(bonkFallspeed < fallspeed && magn[i] < 250) {
        magn[i] += 5;  
      }
  }
  if(sum > (bandPass*200)) {
...
void cmdMusical() {
      switch (subprog) {
         ...
         case 1: { zMIX(); break; }        // 233
         case 2: { zIBeam(); break; }      // 234
         case 3: { zMidland(); break; }    // 235
         case 4: { zMagicFrom(); break; }  // 236
         case 5: { zMagicTo(); break; }    // 237
         case 6: { zFeeryFrom(); break; }  // 238
         ...
         case 20: { ColorWhite(param); strip.show(); break; } // 252 - setting the ribbon color
      }
}
...
void loop(){
  ...
  uint8_t i;
    ...
    if (inComplete) {
      ...
      Serial.println("OK"); // Confirmation - command completed
    }
    else {
      if (prog<29) {
        if ((control&1)!=0) {
    ...
    if (inChar != 253) {
      ...
      if (inChar == 254) {
        ...
        readData[i]=inStr[i];
        inCounter = 0;          // Set the received character counter to the beginning of the input buffer
        inComplete = true;
      }
    } else inCounter = 0;      // Set the received character counter to the beginning of the input buffer
  }
}
